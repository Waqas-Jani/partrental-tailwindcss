    ...
    return (
        <>
            <section className="mt-5 relative z-10">
                <div className="max-w-[1200px] mx-auto px-4  bg-[#F8F6EF] py-10 md:py-16 rounded-lg">
                    ...
                    <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-7 px-5 md:px-10 lg:px-14">
                        {data?.list?.map((item: any, index: number) => (
                            <div key={index} className="group">
                                <Link
                                    href={slug(item.path)}
                                    className={`flex flex-col items-center h-full text-center bg-white ${isIcon ? 'p-5' : 'p-0 pb-5'} rounded-lg relative overflow-hidden transition-all duration-300 before:content-[''] before:absolute before:bottom-0 before:left-0 before:w-0 before:h-1 before:bg-primary before:transition-all before:duration-300 group-hover:before:w-full`}
                                >
                                    <div className={`icon ${isIcon ? 'w-[110px] h-[110px] flex-shrink-0 flex items-center justify-center' : 'w-full h-[200px] md:h-[180px]'}  `}>
                                        <Image
                                    ...
                                    <h3 className={`text-[22px] font-extrabold text-secondary text-center leading-tight mt-5 ${isIcon ? 'px-0' : 'px-5'}`}>
                                        {item.heading}
                                    </h3>
                                </Link>
                            </div>
