...
function LocationMenuItem({ item }: { item: any }) {
    ...
    return (
        <li className="pb-3 last:pb-0">
            <div className="flex items-center gap-2 w-full">
                <ChevronRightIcon cls="h-4 w-4 shrink-0" />
                {hasChildren ? (
                    <button
                        type="button"
                        onClick={() => setIsOpen(!isOpen)}
                        className="flex items-center justify-between font-semibold text-base hover:text-primary transition-colors text-left flex-1 min-w-0"
                    >
                        <span>{parent?.label}</span>
                        <ChevronDownIcon
                            cls={`h-4 w-4 shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`}
                        />
                    </button>
                ) : (
                    <MyLink
                        text={parent?.label}
                        linkType={parent?.type || "internal"}
                        link={parent?.link}
                        cls="font-semibold text-base hover:text-primary transition-colors"
                    />
                )}
            </div>
            {hasChildren && isOpen && (
                <ul className="mt-2 ml-6 space-y-2">
                    {childMenu.map((child: any, idx: number) => (
                        <li key={idx} className="flex items-center gap-2">
                            <ChevronRightIcon cls="h-3 w-3 shrink-0 text-gray-500" />
                            <MyLink
                                text={child.label}
                                linkType={child?.type || "internal"}
                                link={child?.link}
                                cls="text-gray-300 hover:text-primary transition-colors text-sm"
                            />
                        </li>
                    ))}
                </ul>
        ...
        <footer className="bg-black text-primary-gray pb-8">
            <div className="tp-container">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16">
                    ...
                    {/* Posts or Location Menu */}
                    <div>
                        <h3 className="h4-type mb-5">
                            {footer?.recentNewsHeading || "Recent News"}
                        </h3>
                        {footer?.toggleMenu && footer?.locationMenu?.length > 0 ? (
                            <ul className="space-y-3 text-gray-300">
                                {footer.locationMenu.map((item: any, index: number) => (
                                    <LocationMenuItem key={index} item={item} />
                                ))}
                            </ul>
                        ) : (
                            <>
